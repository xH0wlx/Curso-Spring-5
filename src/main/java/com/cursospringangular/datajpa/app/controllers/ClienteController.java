...
public class ClienteController {
	...
	public ResponseEntity<Resource> verFoto(@PathVariable String filename) {
		...
		// inline puede cambiarse por attachment; filename=\"" + recurso.getFilename() +"\"
		return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "inline").body(recurso);
	}
	...
	public String listar(@RequestParam(name = "page", defaultValue = "0") int page, Model model,
		...
		if( securityContext.isUserInRole("ROLE_ADMIN") ) {
			...
			logger.info("Forma usando la clase SecurityContextHolderAwareRequestWrapper: Hola ".concat(auth.getName()).concat(" NO tienes acceso."));
		}
		
		if( request.isUserInRole("ROLE_ADMIN") ) {
			logger.info("Forma usando la clase HttpServletRequest: Hola ".concat(auth.getName()).concat(" tienes acceso."));
		}else{
			logger.info("Forma usando la clase HttpServletRequest: Hola ".concat(auth.getName()).concat(" NO tienes acceso."));
		}
